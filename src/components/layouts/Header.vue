<template>
  <header class="flex justify-between mt-12 w-full h-9">
    ...
    <div class="flex gap-14 items-center cursor-default dark:text-[#DBDBDB]">
      <span>{{ username }}</span>
      <button class="transition duration-200" @click.prevent="toggleDarkMode">
        <svg v-if="!settingsStore.darkMode" xmlns="http://www.w3.org/2000/svg" class="w-7 h-7 hover:text-yellow-400 text-yellow-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
        <svg v-else class="w-6 h-6 hover:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
